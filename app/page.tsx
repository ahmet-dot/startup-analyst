...
const assessmentConfig = {
  Strong: { color: '#10b981', bg: 'rgba(16,185,129,0.1)', border: 'rgba(16,185,129,0.25)', label: 'Strong' },
  Moderate: { color: '#f59e0b', bg: 'rgba(245,158,11,0.1)', border: 'rgba(245,158,11,0.25)', label: 'Moderate' },
  Weak: { color: '#ef4444', bg: 'rgba(239,68,68,0.1)', border: 'rgba(239,68,68,0.25)', label: 'Weak' },
}
...
function SectionCard({ section, index }: { section: AnalysisSection; index: number }) {
  ...
  const config = assessmentConfig[section.assessment]
  ...
  return (
    <div
      ...
      style={{
        ...
        opacity: 0,
        animation: `sectionReveal 0.5s ease ${index * 0.08}s forwards`,
      }}
    >
      <button
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            <span style={{ fontFamily: 'var(--font-body)', fontWeight: 600, color: 'var(--text)', fontSize: 14 }}>
              {section.title}
            </span>
            <span style={{
      ...
      {expanded && (
        <div style={{ padding: '0 20px 18px', borderTop: '1px solid var(--border)' }}>
          <p style={{
            fontSize: 14, lineHeight: 1.75, color: 'rgba(232,232,240,0.82)',
            paddingTop: 14, fontFamily: 'var(--font-body)',
          }}>
            {section.content}
          </p>
        </div>
  ...
  const analyze = async () => {
    ...
    try {
      ...
      while (true) {
        ...
        setStreamText(rawBuffer.current)
      }

      // Parse the complete JSON
      const parsed: AnalysisResult = JSON.parse(rawBuffer.current)
      setResult(parsed)
      setPhase('done')
    } catch (err) {
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 800, margin: '0 auto', padding: '0 24px' }}>
        ...
        <header style={{ paddingTop: 60, paddingBottom: 48, textAlign: 'center' }}>
          <div style={{
            ...
            <span style={{ fontSize: 12, color: '#818cf8', letterSpacing: '0.08em', fontWeight: 500 }}>
          ...
          <h1 style={{
            ...
            <span style={{ color: 'var(--text)' }}>Is your startup idea</span>
            <br />
        ...
        {phase === 'idle' && (
          <section style={{
            ...
            <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--muted)', letterSpacing: '0.06em', marginBottom: 12 }}>
              YOUR STARTUP IDEA
            </label>
            <textarea
              ref={textareaRef}
              ...
              placeholder="Describe your startup idea in detail. Include the problem you're solving, your target market, how you plan to make money, and what makes it different from existing solutions..."
              rows={6}
              style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
              <span style={{ fontSize: 12, color: charCount > maxChars * 0.8 ? '#f59e0b' : 'var(--muted)' }}>
                {charCount} / {maxChars} chars · Cmd+Enter to submit
              </span>
              <button
                onClick={analyze}
                ...
                style={{
                  ...
                  background: idea.trim().length < 10 ? 'rgba(99,102,241,0.2)' : '#6366f1',
                  color: idea.trim().length < 10 ? 'rgba(255,255,255,0.3)' : '#fff',
                  border: 'none', cursor: idea.trim().length < 10 ? 'not-allowed' : 'pointer',
        ...
        {phase === 'streaming' && (
          <section style={{
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <p style={{ color: 'var(--muted)', fontSize: 14 }}>Conducting deep market analysis...</p>
            </div>
            ...
            <div style={{ marginTop: 20, fontSize: 12, color: 'var(--muted)', textAlign: 'left' }}>
              {streamText.length > 0 && (
                <span>Received {streamText.length} chars of analysis...</span>
              )}
            </div>
          </section>
        )}
          ...
          <div style={{ paddingBottom: 80 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 20 }}>
              <button
                ...
                onMouseEnter={e => { (e.target as HTMLElement).style.color = 'var(--text)'; (e.target as HTMLElement).style.borderColor = 'rgba(255,255,255,0.2)'; }}
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 200 }}>
                  <div style={{ fontSize: 11, color: 'var(--muted)', letterSpacing: '0.08em', fontWeight: 600, marginBottom: 10 }}>
                    IDEA SUMMARY
                  </div>
                  <p style={{ fontSize: 15, lineHeight: 1.7, color: 'var(--text)', fontFamily: 'var(--font-body)' }}>
                    {result.summary}
                  </p>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              ...
              <div style={{
                ...
                animation: 'sectionReveal 0.5s ease 0.6s forwards', opacity: 0,
              }}>
                <div style={{ fontSize: 12, color: '#10b981', letterSpacing: '0.08em', fontWeight: 600, marginBottom: 14 }}>
                  ✅ TOP STRENGTHS
                </div>
              ...
              <div style={{
                ...
                borderRadius: 12, padding: 20,
                animation: 'sectionReveal 0.5s ease 0.7s forwards', opacity: 0,
              }}>
                <div style={{ fontSize: 12, color: '#ef4444', letterSpacing: '0.08em', fontWeight: 600, marginBottom: 14 }}>
                  🚩 TOP CONCERNS
                </div>
                <ol style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {result.pivots && (
              <div style={{
                ...
                animation: 'sectionReveal 0.5s ease 0.8s forwards', opacity: 0,
              }}>
                <div style={{ fontSize: 12, color: '#f59e0b', letterSpacing: '0.08em', fontWeight: 600, marginBottom: 12 }}>
                  🔄 STRATEGIC PIVOTS
                </div>
                <p style={{ fontSize: 14, color: 'rgba(232,232,240,0.85)', lineHeight: 1.7, fontFamily: 'var(--font-body)' }}>
                  {result.pivots}
                </p>
              </div>
            )}
            ...
            <div style={{
              ...
              borderRadius: 12, padding: 20,
              animation: 'sectionReveal 0.5s ease 0.9s forwards', opacity: 0,
            }}>
              <div style={{ fontSize: 12, color: '#818cf8', letterSpacing: '0.08em', fontWeight: 600, marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {result.nextSteps.map((step, i) => (
                  <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                    ...
                    <p style={{ fontSize: 14, color: 'rgba(232,232,240,0.85)', lineHeight: 1.65, fontFamily: 'var(--font-body)', paddingTop: 3 }}>
                      {step}
                    </p>
                  </div>
                ))}
        ...
        <footer style={{ paddingBottom: 32, paddingTop: 16, textAlign: 'center', borderTop: '1px solid var(--border)' }}>
          <p style={{ fontSize: 12, color: 'var(--muted)' }}>
            Built with Claude AI · StartupAnalyst-GPT
          </p>
        </footer>
      </div>
    </main>
